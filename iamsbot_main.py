...
NOTISTATE_ACTION=[]
menucoinmarkup = {
    'keyboard': [['BACK'], ['BTC', 'BCH'], ['LTC', 'OMG'], ['BCH', 'DASH', 'XZC'], ['GNO', 'REP', 'XRP']]}
menuexchange = {'keyboard': [['BACK'], ['BXINTH', 'TDAX'], ['BITTREX', 'YOBIT']]}
mainmenu = {'keyboard': [['BUY', 'SELL', 'INFO'], ['CANCEL ORDER'], ['NOTIFICATION', 'STRATEGY'], ['CANCEL']]}
...
class YourBot(telepot.Bot):
    ...
    def coin_buysalcan(self, chat_id, bxin, Coin):
        ...
        if 'buyCoin' in BX:
            ...
            if Bal == 201:
                ...
                bot.sendMessage(chat_id, str("THB balance is not available.!!"))
                BX.clear()
            else:
            ...
            if Bal == 201:
                print(Coin + " coin balance not available.!!")
                bot.sendMessage(chat_id, str(Coin + " coin is balance not available.!!"))
                BX.clear()
            else:
                bot.sendMessage(chat_id, str(Bal))
    ...
    def coinmenu(self, chat_id):
        ...
        COINLISTINFO.clear()
        ##############
        bot.sendMessage(chat_id, "|== TBOT MENU  ==|\
            \n\U0001F4B5 /buyCoin  \
            \n\U0001F4B5 /saleCoin \
            ...
            \n\U0001F4B5 /cancel", reply_markup=mainmenu)
               ...
               # self.check_close_order('bxinth',chat_id)
                    ...
                    if is_number(Rate) == True and Rate != "0":
                        ...
                        if is_number(Oid) == True:  ## Test
                            bot.sendMessage(chat_id, "Sell:" + Coin + "\nAmount:" + str(Sell) + "  \nRate:" + str(
                                Rate) + " Bath \n")
                            # if simtest == "yes":
                            # ST = ck_close_order_sim(Oid, Coin, 'sell', 'bxinth')
                            # else:
                            #    ST = ck_close_order(bxin,Oid, Coin, 'sell', 'bxinth')
                            #if ST == True:
                            #    bot.sendMessage("Sell Order" + str(Oid) + " --> Completed")
                            #else:
                            ...
                            if simtest != "yes":
                                ...
                                if Bal == 201:
                                    print(symbol + " balance is not available.!!")
                                    bot.sendMessage(chat_id, str(symbol + " balance is not available.!!"))
                                    BX.clear()
                    ...
                    if is_number(Rate) == True and Rate != "0":
                        ...
                        if is_number(Oid) == True:  ## Test
                            ...
                            if Bal == 201:
                                ...
                                # if simtest == "yes":
                                # ST = ck_close_order_sim(Oid, Coin, 'buy', 'bxinth')
                                #else:
                                #    ST = ck_close_order(bxin, Oid, Coin, 'buy', 'bxinth')

                                #if ORDERBUY[2] <= LastPrice:
                                #   bot.sendMessage(chat_id,"Buy Order " +str(Oid) +" --> Successfully")
                                #else:  ## Open Order only wait for apply
                                bot.sendMessage(chat_id, "Open Order " + str(Oid) + "  --> Successfully")
                                BX.clear()
                ...
                if chat_id in setmail:
                    ...
                    try:
                        ...
                        # else:
                        # bot.sendChatAction(chat_id, 'typing')
                        #    interact = '\U00002753 May I help you,guy '
                        #    bot.sendMessage(chat_id, interact)
                        #    bot.sendMessage(chat_id, "\U0001F4D8 Type /menu command for list the Menu")
                ...
                if msg['text'] == '/strategy' or msg['text'] == "STRATEGY":
                    ...
                    if is_number(Uid) == True:
                        if "stoploss_sell" in STRATEGY:
                            ...
                            ST2 = Update_CutLoss(Uid, 'bxinth', BuyStopLoss)
                            if ST1 == "OK" and ST2 == "OK":
                                bot.sendMessage(chat_id, "Update New value Completed !!", reply_markup=mainmenu)
                            else:
                                bot.sendMessage(chat_id, "Update New value Failed !!", reply_markup=mainmenu)
            ...
            if is_number(lastprice) == True and order_id != 0 and order_id != None:
                ...
                if result != None:
                    if result[0] == "StopLossUpdate":
                        ...
                        bot.sendMessage(chat_id, "Update StopPoint BTS \
                                         \nBuy " + str(qty) + \
                                        "\nRate " + str(rate) + \
                                        "\nNow LastPrice:" + str(lastprice) + \
                                        "\nNow StopPoin(" + str(result[2]) + " %):" + str(StopLoss_Point) + \
                                        "\nChange Fee:" + str(fee) + \
                                        "\nNow Profit:" + str(format_float(profit_fee)) + " Bath",
                                        reply_markup=mainmenu)
                    ...
                    if result[0] == "CutLoss":
                        ...
                        if ST == "Noti" and allow_cutloss_bts == "NO":
                            ...
                            NOTIINFO+=("\nPlease Take Action Cut Loss Now!! \
                                     ...
                                     \nBuy " + str(qty) + "\
                                     \nSold " + str(format_float(((CutLossPrice / rate) * qty))) + "\
                                     \nChange Fee " + str(fee) + "\
                                     \nProfit " + str(format_float(profit_fee)) + " Bath \
                                     \nAccept this Action or not,/OK")
                            #time.sleep(4)
                        ...
                        if ST == "Noti" and allow_stoploss_bts == "NO":
                            NOTISTATE_ACTION.clear()
        ...
        for order in list(ST):
            ...
            if is_number(lastprice) == True and order_id != 0 and order_id != None:
                ...
                if result != None:
                    if result[0] == "StopLossUpdate":
                        StopLoss_Point = result[1]
                        print("!!! Update StopLoss Point STS to " + str(StopLoss_Point))
                        profit = (((StopLoss_Point / rate) * qty) - qty)
                        profit_fee = (profit - (profit * fee))
                        bot.sendMessage(chat_id, "Update StopPoint STS \
                    ...
                    if result[0] == "CutLoss":
                        ...
                        if simtest == "yes":
                            ...
                            if noti_sts == "ON" and allow_cutloss_sts != "YES":
                                ...
                                ST = sale_coin(bxin, coin, float(volumn), float(CutLossPrice), exchange)
                        if ST == "Noti" and allow_cutloss_sts == "NO":
                            profit = (((CutLossPrice / rate) * qty) - qty)
                            profit_fee = (profit - (profit * fee))
                            NOTIINFO+=(
                        ...
                        if ST == "Noti" and allow_stoploss_sts == "NO":
                            profit = (((LastPrice / rate) * qty) - qty)
                            fee = 0.0025
                            profit_fee = (profit - (profit * fee))
                            NOTIINFO+=(
...
while True:
   ...
   #simtest="yes"
      ...
      if NOTIINFO != "":
         for adminid in adminchatid:
             bot.sendMessage(adminid,NOTIINFO)
             NOTIINFO=""
      time.sleep(12)
